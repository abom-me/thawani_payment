...
class SavedCardsScreen extends StatefulWidget {
  const SavedCardsScreen({super.key, required this.saved, required this.apiKey, required this.amount, required this.returnLink, required this.testMode, this.metadata, required this.onCancelled, required this.onPaid});
...
thawaniPayment.create(widget.apiKey, amount: widget.amount, returnLink: widget.returnLink, testMode: widget.testMode, cardID: data[i].id!, clientID: data[i].customer!.customerClientId!, onDone: (c,d){

  Navigator.push(context, MaterialPageRoute(builder: (_)=>PaySavedWidget(paid: (data){
    widget.onPaid(data);
  }, unpaid: (data){widget.onCancelled(data);}, url: c.data!.nextAction!.url!, api: widget.apiKey, testMode: widget.testMode, returnLink: widget.returnLink, payID:c.data!.id! ,)));
}, onError: (d){});
            },
            child: Container(
...
Row(
  ...
  children: [
    Container(
      ...
      child: SvgPicture.string(rawSvg,color:const Color(0xff80be59)),
    ),

    Text("Thawani Card",style: TextStyle(color: Colors.white,fontSize: 20,fontWeight: FontWeight.bold),),
    Text(data[i].brand??"",style: const TextStyle(color: Colors.white,fontSize: 17,fontWeight: FontWeight.bold),),

  ],
),
                  Align(
                    alignment: Alignment.center,
                    child: Text(data[i].maskedCard??"",style: TextStyle(color: Colors.white,fontSize: 20,fontWeight: FontWeight.bold),),
                  ),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text(data[i].nickname??"",style: const TextStyle(color: Colors.white,fontSize: 17,fontWeight: FontWeight.bold),),
                      Text("EXP: ${data[i].expiryMonth} / ${data[i].expiryYear}",style: const TextStyle(color: Colors.white,fontSize: 15,),),
                    ],
                  )
                ],
              )
