...
class ThawaniPayBtn extends StatefulWidget {
  const ThawaniPayBtn(
      ...
      required this.onPaid, this.child,
        required this.pKey,  this.metadata, required this.clintID,
        this.buttonStyle, this.testMode, this.onError,  this.successUrl,  this.cancelUrl})
      : super(key: key);
...
class _ThawaniPayBtnState extends State<ThawaniPayBtn> {
  late ButtonStyle buttonStyle=widget.buttonStyle??  ButtonStyle(
    elevation: MaterialStateProperty.all(0),
    shape: MaterialStateProperty.all(RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
    backgroundColor: MaterialStateProperty.all(const Color(0xff4FB76C)),
  );
  late Widget child = widget.child??const Text("Pay",style: TextStyle(color: Colors.white,fontSize: 17),);
  ...
  Widget build(BuildContext context) {
    return ElevatedButton(
      style:buttonStyle ,
        onPressed: () async {
          dataBack = await RequestHelper.postRequest(api,
              {
                ...
                MaterialPageRoute(
                    builder: (context) => PayWidget(
                      api: widget.api,
                        ...
                        url: testMode==true?'https://uatcheckout.thawani.om/pay/${dataBack['data']['session_id']}?key=${widget.pKey}':'https://checkout.thawani.om/pay/${dataBack['data']['session_id']}?key=${widget.pKey}' ,
                          paid: (statusClass) {
